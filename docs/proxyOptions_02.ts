...
// Centralized Frappe Server IP Configuration
// Local Development Frappe IP: "172.16.134.191"
// Server Frappe IP: "172.16.131.206" (set via VITE_APP_BACKEND_HOST in .env when uploading/deploying to server)
const FRAPPE_SERVER_IP = env.VITE_APP_BACKEND_HOST || "172.16.134.191";

export default {
  "^/(app|api|assets|files|private)(/|$)": {
    // Old Server Frappe IP target (Commented out):
    // target: `http://172.16.131.206:${webserver_port}`,
    // Local Development Frappe IP target:
    target: `http://${FRAPPE_SERVER_IP}:${webserver_port}`,
    ws: true,
    // CRITICAL ADDITION: This changes the Host header to match the target
    ...
    router: function (req: string) {
      // Old Server Frappe IP router (Commented out):
      // return `http://172.16.131.206:${webserver_port}`;
      // Local Development Frappe IP router:
      return `http://${FRAPPE_SERVER_IP}:${webserver_port}`;
    },
  },
  // Proxy for external Ledger API to avoid CORS
  "/ledger-api": {
