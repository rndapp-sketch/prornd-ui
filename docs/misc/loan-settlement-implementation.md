...
- `FundReceivedDetails.tsx` (the read-only view) gets a small **read-only** "Loan Settlement(s) Requested" info block listing every linked `Loan Settlement` doc (fetched by `fund_received_reference = <this doc>`) — informational only, no editing here.
...
- **Route**: `main.tsx` → `path: "loan-settlement/:id"` → `<LoanSettlementDetails />`.
...
**Frontend** (`src/`)
- `pages/AddFundReceived.tsx` — new blocking `LoanSettlementModal` component (loan list, select-all, per-loan full/partial, per-head return table, Settle the Loan / Keep Existing, plus the Edit re-open flow of §1.5a), surfacing §10.1's re-validation errors inline; `prorate()` mirroring the server split (§4a.1); `applySettlementRequirements()` prefilling and locking the form (§6); `doc_data.loan_settlement_refs` appended at final submit (§5.3/§6).
- `pages/FundReceivedDetails.tsx` — read-only "Loan Settlement(s) Requested" info block, incl. the head-wise return table per settlement.
- `pages/LoanSettlementDetails.tsx` (new) — staff, RnD processing screen (§8) + `publish_status` badge and the "Retry Publish" action (§10.3/§10.4).
- `main.tsx` — new route.
