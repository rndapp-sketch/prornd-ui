...
const MemoizedTransactionsTable = memo(
    ({ tableData, onRowChange, onFileChange, onAddRow, onDeleteRow, total, target }: any) => {
        // `target` is the Fund Received Amount; the transaction rows must add up to it.
        const hasTarget = typeof target === "number" && !isNaN(target) && target > 0;
        const diff = hasTarget ? (total || 0) - target : 0;
        const balanced = hasTarget && Math.abs(diff) < 0.01;
        return (
            <div>
...
const AddFundReceived: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            // ── Fund Received Amount = Σ transactions = Σ budget breakup ──
        ...
        try {
            ...
            // Loan settlement linkage — an extra, non-schema key. The backend pops it
            // off doc_data before field-mapping, so nothing is ever written onto the
            // Fund Received doctype itself. Absent only when the project had no loans —
            // settlement is otherwise mandatory, so the modal cannot be skipped.
                                        ...
                                        {section.title ===
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                <MemoizedTransactionsTable
                                                    ...
                                                    total={totalTransactionAmt}
                                                    target={fundReceivedAmt}
                                                />
                                    ...
                                    {validationState.totalValidation.isValid &&
                                        ...
                                        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/40 rounded-md p-2.5 text-center">
                                            ...
                                            <p className="text-[11px] text-blue-600 dark:text-blue-400 mt-1">
                                                {!isFundAmtBreakupValid
                                                    ? transactionAmtError && fundReceivedAmtError
                                                        ? "Transaction Details and Budget Breakup must both total the Fund Received Amount"
                                                        : transactionAmtError
                                                            ? "Transaction Details must total the Fund Received Amount"
