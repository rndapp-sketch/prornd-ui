...
const FrappeButton = ({ children, className, onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) => (
    <button
        onClick={onClick}
        className={cn(
            "px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg font-semibold text-zinc-900 dark:text-zinc-100 shadow-[2px_2px_0px_rgba(0,0,0,0.25)] transition-all", // font-bold changed to font-semibold
