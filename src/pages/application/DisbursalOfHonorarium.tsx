...
const DisbursalOfHonorarium: React.FC = () => {
    ...
    const fetchList = useCallback(async () => {
        setLoading(true);
        try {
            const response = await fetch('/api/resource/Disbursal of Honorarium?fields=["name","creation","workflow_state","total_amount","project_number","date_of_request","department","webmail_id","name_of_applicant","owner"]&order_by=creation desc&limit_page_length=0');
                            ...
                            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                                {disbursalList.map((item) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100">{item.name_of_applicant || item.webmail_id || item.owner}</td>
                                        <td className="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">
                                            {item.department ? <DepartmentName name={item.department} /> : '-'}
                                        </td>
