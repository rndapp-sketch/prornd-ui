...
import { CalendarIcon, UserIcon, EditIcon, FileSpreadsheetIcon as LedgerIcon } from "lucide-react";
...
const FrappeButton = ({ children, onClick, disabled, className, variant = 'ghost' }: {
    ...
    <button
        ...
        className={cn(
            ...
            variant === 'ghost' && "bg-transparent text-zinc-900 dark:text-zinc-100 hover:bg-zinc-200 dark:hover:bg-zinc-700",
...
const DisbursalOfHonorariumDetails: React.FC = () => {
    ...
    const { call: fetchDoc } = useFrappePostCall<{ message: DisbursalData }>('frappe.client.get');
    ...
    const projectTitle = data?.project_number || data?.project_name || data?.project_title || "";
    ...
    const projectAmountsResult = (projectAmountsData as any)?.message?.data ?? (projectAmountsData as any)?.data ?? {};
    ...
    const loadData = async () => {
        ...
        setLoading(true);
        try {
            const docRes = await fetchDoc({ doctype: "Disbursal of Honorarium", name: id });
            if (docRes?.message) {
    ...
    const handleSubmit = async () => {
        ...
        if (!confirm("Are you sure you want to submit this Disbursal of Honorarium? This action cannot be undone.")) return;
    ...
    const totalAmount = data?.table_weoy?.reduce((sum: number, row: any) => {
        const amt = parseFloat(row.amount || 0);
        return sum + (isNaN(amt) ? 0 : amt);
    }, 0) || data?.total_amount || 0;
            ...
            <main className="transition-all duration-300 ease-in-out p-6 md:p-10">
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        {(data.workflow_state === 'Draft' || !data.workflow_state) && id && (
                            <>
                                <button
                        ...
                        {id && <DisbursalOfHonorariumActionButtons 
                            docname={id} 
                            onActionComplete={() => loadData()}
                            blockedActions={isRnDStaff && !isCommitted ? {
                                actions: ['Forward', 'Approve', 'Hos', 'AR', 'Approve & Forward'], 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <FrappeCard title="Project Details" className="border-t-4 border-t-[#D97757]">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Project Number</label>
                                    <div className="font-semibold text-zinc-900 dark:text-zinc-100">{data.project_number || "-"}</div>
                                </div>
                                <div>
                                    ...
                                    <div className="font-semibold text-zinc-900 dark:text-zinc-100">{data.project_name || data.project_title || "-"}</div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Account Head</label>
                                    <div className="font-semibold text-zinc-900 dark:text-zinc-100">{resolvedAccountHead || data.account_head || "-"}</div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Total Amount</label>
                        ...
                        <FrappeCard title="Applicant Details">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Applicant Name</label>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Webmail ID</label>
                                    <div className="font-semibold text-zinc-900 dark:text-zinc-100">{data.webmail_id || "-"}</div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Designation</label>
                                    <div className="font-semibold text-zinc-900 dark:text-zinc-100">{data.designation_of_applicant || "-"}</div>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Date of Request</label>
                                    <div className="font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                                        <CalendarIcon className="w-4 h-4 text-zinc-400 dark:text-zinc-500" />
                                        {data.date_of_request ? new Date(data.date_of_request).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : "-"}
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <table className="min-w-full divide-y divide-zinc-300 dark:divide-zinc-700">
                                        <thead className="bg-zinc-200 dark:bg-zinc-700">
                                            <tr className="divide-x divide-zinc-300 dark:divide-zinc-600">
                                                <th className="px-4 py-3 text-left text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase">Sl.</th>
                                                <th className="px-4 py-3 text-left text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase">Web Mail ID</th>
                                                <th className="px-4 py-3 text-left text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase">Name</th>
                                                <th className="px-4 py-3 text-left text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase">Emp ID</th>
                                                <th className="px-4 py-3 text-left text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase">Designation</th>
                                                <th className="px-4 py-3 text-left text-sm font-bold text-zinc-900 dark:text-zinc-100 uppercase">Dept/Section</th>
                                        ...
                                        <tbody className="divide-y divide-zinc-300 dark:divide-zinc-700 bg-white dark:bg-zinc-900">
                                            {data.table_weoy.map((row: any, index: number) => (
                                                <tr key={row.name || index} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 divide-x divide-zinc-300 dark:divide-zinc-700">
                                                    <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100">{index + 1}</td>
                                                    <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100">{row.web_mail_id || "-"}</td>
                                                    <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100 font-medium">{row.name1 || "-"}</td>
                                                    <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100 font-mono">{row.emp_id || "-"}</td>
                                                    <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100">{row.designation || "-"}</td>
                                                    <td className="px-4 py-3 text-sm text-zinc-900 dark:text-zinc-100">{row.department_section || "-"}</td>
                                        ...
                                        <tfoot className="bg-claude-bg dark:bg-zinc-900 border-t-2 border-zinc-300 dark:border-zinc-700">
                                            <tr>
                                                <td colSpan={6} className="px-4 py-3 text-sm font-bold text-zinc-900 dark:text-zinc-100 text-right uppercase">
                                                    Total Amount:
                                                </td>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-zinc-900 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                            ...
                            {id && <ActivityStream doctype="Disbursal of Honorarium" docname={id} />}
                        </div>
                        ...
                        {(data.workflow_state === "Pending Staff Approval" || data.workflow_state === "Approved") &&
                            isRnDStaff &&
                            !isCommitted && (
                                <div className="bg-white dark:bg-zinc-900 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                                    <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-4">
                                ...
                                <div className="bg-white dark:bg-zinc-900 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                                    ...
                                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 flex flex-col gap-1">
                                        <p className="text-xs text-blue-600 font-semibold uppercase tracking-wide">
                                            {cachedCommitment ? "Commitment Initiated" : "Linked Commitment"}
                                        </p>
                                        <div className="flex justify-between items-end">
                                            <p className="text-sm font-medium text-blue-900">
                                                {cachedCommitment
                                                    ? (budgetHeadList.find(b => b.id === (cachedCommitment.accountHeadId || cachedCommitment.budget_head))?.name || 
                                                       cachedCommitment.accountHeadId || 
                                                       cachedCommitment.budget_head)
                                                    : linkedCommitment?.head}
                                            </p>
                                            <p className="text-lg font-bold text-blue-700">
                                                ₹ {Number(cachedCommitment ? (cachedCommitment.commitAmount || cachedCommitment.commit_amount) : (linkedCommitment?.committed || 0)).toLocaleString("en-IN")}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <FrappeCard>
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Workflow State</label>
                                    <div className="font-medium">
                                        <span className={cn(
                                            "inline-flex px-3 py-1 text-xs font-bold rounded-full",
                                            data.workflow_state === "Approved" && "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400",
                                            data.workflow_state === "Rejected" && "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400",
                                            data.workflow_state === "Draft" && "bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300",
                                            !["Approved", "Rejected", "Draft"].includes(data.workflow_state || '') && "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                                        )}>
