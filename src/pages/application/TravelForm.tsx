...
const TravelForm: React.FC = () => {
    ...
    // Handle field changes with side effects (like auto-filling traveler details)
    const handleFieldChangeWithSideEffects = useCallback(async (fieldname: string, value: any) => {
        handleChange(fieldname, value);

        // Auto-fill traveler details when traveler is selected
        if ((fieldname === 'traveler_id' || fieldname === 'if_traveler') && value) {
            try {
                ...
                if (result?.message) {
                    ...
                    setFormData(prev => ({
                        ...prev,
                        [fieldname]: value,
                        traveler_designation: details.designation || '',
                        traveler_department: details.applicant_department || '',
                    }));
                }
            } catch (err) {
        ...
        // Clear traveler details if traveler is cleared
        if ((fieldname === 'traveler_id' || fieldname === 'if_traveler') && !value) {
            setFormData(prev => ({
                ...prev,
                [fieldname]: '',
                traveler_designation: '',
                traveler_department: '',
            }));
        }
    ...
    }, [handleChange, fetchUserDetails, fetchUserDetailsByEmail]);
