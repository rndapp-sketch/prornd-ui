...
import { Save, Send, UserIcon, IndianRupeeIcon } from 'lucide-react';
...
const MiscellaneousCommitForm: React.FC = () => {
    ...
    // The project query param (and formData.project_number) is the Project Registration
    // doc name — resolve it to the human-readable project number for display.
    const resolvedProjectNo = useMemo(
        () => linkOptions.project_number?.find(o => o.value === formData.project_number)?.label || formData.project_number || projectParam,
        [linkOptions, formData.project_number, projectParam],
    );
