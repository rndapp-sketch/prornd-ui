...
import { ArrowLeft, Save, Loader2, User, CheckCircle2 } from "lucide-react";
import DynamicFormRenderer from "@/components/forms/DynamicFormRenderer";
import { universalUserAPI, prepareFormDataForApi } from "@/services/apiService";
import { Skeleton } from "@/components/ui/skeleton";

export default function UniversalUserForm() {
    const { id } = useParams();
    ...
    // API Hooks
    const { call: getFieldsCall } = useFrappePostCall<{ message: any }>(universalUserAPI.getFields);
    const { call: saveCall } = useFrappePostCall<{ message: any }>(universalUserAPI.save);
    ...
    const fetchFormConfiguration = useCallback(async () => {
        ...
        try {
            ...
            if (response && response.message) {
                ...
                // Default status
                let initialData: any = { status_u_r: "Active" };
        ...
        try {
            ...
            if (response && response.message) {
                ...
                if (message.status === "success") {
                    ...
                    alert(`Universal User saved successfully. (ID: ${message.docname})`);

                    // Navigation to edit mode if we were creating a new one
                    if (!savedDocName) {
                        navigate(`/universal-user/${message.docname}`, { replace: true });
                    }
                } else {
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-sans text-zinc-500 dark:text-zinc-400 max-w-2xl">
                                {savedDocName
                                    ? `Editing universal user profile ${savedDocName}`
                                    : "Create a new universal user account."}
                            </p>
                        </div>
                    </div>
                ...
                <Card className="border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden bg-white dark:bg-zinc-900">
                    <CardHeader className="bg-[#FDFDFD] dark:bg-[#27272A] border-b border-zinc-100 dark:border-zinc-800 px-8 py-6">
                        ...
                        <CardDescription className="text-base text-zinc-700 dark:text-zinc-300 font-medium font-serif">
                            Fill out the user information fields below. All mandatory fields are marked with an asterisk (*).
                        </CardDescription>
                    </CardHeader>
                            ...
                            <div className="sticky bottom-0 border-t border-zinc-200 dark:border-zinc-800 bg-[#FDFDFD]/95 dark:bg-zinc-900/95 backdrop-blur-md px-8 py-5 flex items-center justify-between z-10 transition-all shadow-[0_-4px_24px_-12px_rgba(0,0,0,0.1)]">
                                <div className="text-sm text-zinc-500 dark:text-zinc-400 font-medium">
                                    <span className="flex items-center gap-2">
                                        ...
                                        Profile in {formData.status_u_r || "Draft"} Mode
                                    </span>
                                </div>
