...
const Reimbursement: React.FC = () => {
    ...
    const isOverheadProject = useIsOverheadProject(formData.project_number || formData.project_name);
    ...
    useEffect(() => {
        const loadFormAndDocument = async () => {
            if (formDataResult?.message && !dataLoaded) {
                ...
                // The backend only pre-packages Principal Investigator users into
                // the "PI Webmail Id" options. Merge in head_approver_1 (department
                // head), project staff, and Student users too so they also appear
                // in that dropdown.
                try {
                    const extraRoles = ['head_approver_1', 'project staff', 'Student'];
                    const extraResults = await Promise.all(
                        extraRoles.map((role) =>
                            // Query the User doctype directly with a child-table filter on its
                            // "roles" table (Has Role). Permission is checked against User (which
                            // regular applicants can list), unlike querying "Has Role" directly,
                            // which is restricted to System Manager and returns 403 for them.
                            getListCall({
                                doctype: 'User',
                                filters: [['Has Role', 'role', '=', role]],
                                fields: ['name', 'full_name'],
                                limit_page_length: 0,
                            }).catch((e) => {
                                console.error(`Failed to load "${role}" users for PI Webmail Id:`, e);
                                return null;
                            }),
                        ),
                    );
                    console.log('extra-role User results:', extraResults);

                    const extraOptions: LinkOption[] = extraResults
                        .flatMap((res: any) => res?.message || [])
                        .map((u: any) => ({
                            value: u.name,
                    ...
                    if (extraOptions.length) {
                        setLinkOptions(prev => {
                            const mergeInto = (key: string) => {
                                ...
                                const merged = new Map<string, LinkOption>();
                                [...existing, ...extraOptions].forEach(opt => merged.set(opt.value, opt));
                                return Array.from(merged.values());
                            };
                            return {
                                ...prev,
                                reimb_applying_for_mail: mergeInto('reimb_applying_for_mail'),
                                User: mergeInto('User'),
                            };
                        });
                    }
                } catch (e) {
                    console.error('Failed to load extra-role users for PI Webmail Id:', e);
                }
