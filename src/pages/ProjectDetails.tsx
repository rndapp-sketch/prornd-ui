...
const LogWorkflowActions = ({ docname, onAction, isLoading: isActionLoading }: { docname: string, onAction: (action: string) => void, isLoading: boolean }) => {
  const { data, error, isLoading } = useFrappeGetCall<{ message: string[] }>( // Changed to string[]
    'rndopsapp.rndopsapp.doctype.project_registration.project_registration.get_available_workflow_actions',
    { docname }
  );
console.log('✅ Allowed Actions (from LogWorkflowActions):', docname);
  ...
  return (
    <div className="flex items-center space-x-2 no-print">
      {data.message.map((actionString: string) => ( // Map over strings
        <Button
          key={actionString}
          onClick={() => onAction(actionString)}
          className={getButtonClass(actionString)}
          disabled={isActionLoading}
        >
          {isActionLoading ? 'Processing...' : actionString}
        </Button>
      ))}
    </div>
  );
