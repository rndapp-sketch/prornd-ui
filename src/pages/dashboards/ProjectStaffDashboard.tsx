              ...
              <section className="mb-6 bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-bold text-[#3F3F46] dark:text-[#E4E4E7]">Recent Submissions</h2>
                  </div>
                ...
                {trackingLoading ? (
                  ...
                  <div className="py-8 text-center text-[#A1A1AA]">
                    ...
                    <p className="text-xs mt-1">Applications you submit will appear here.</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <span className={cn("px-2 py-0.5 rounded text-[10px] font-bold border", getStatusStyle(item.workflow_state))}>
                              {item.workflow_state}
                            </span>
                            <span className="text-[10px] text-[#A1A1AA] font-medium">{item.doctype}</span>
                          </div>
                          <p className="text-sm font-medium text-[#3F3F46] dark:text-[#E4E4E7] truncate">
                            {item.id}
                          </p>
                          <p className="text-[11px] text-[#A1A1AA] mt-0.5">
                            Modified {formatRelativeTime(item.modified)} · Created {new Date(item.creation).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                          </p>
                        </div>
                        <button
              ...
              <div className="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm overflow-hidden">
                {trackingLoading ? (
                  <div className="py-12 text-center text-[#A1A1AA]">
                    ...
                    <p className="text-sm font-medium">Loading applications list...</p>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700">
                        <TableRow>
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 h-10 px-4">Application Type</TableHead>
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 h-10 px-4">Reference ID</TableHead>
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 h-10 px-4">Status</TableHead>
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 h-10 px-4">Date Created</TableHead>
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 h-10 px-4">Last Modified</TableHead>
                          <TableHead className="font-bold text-xs uppercase tracking-wider text-zinc-500 dark:text-zinc-400 h-10 px-4 text-right">Actions</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {paginatedItems.map((item) => (
                          <TableRow key={item.id} className="border-b border-zinc-100 dark:border-zinc-700/50 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                            ...
                            <TableCell className="px-4 py-3">
                              <span className={cn("px-2 py-0.5 rounded-full text-[11px] font-bold border inline-block", getStatusStyle(item.workflow_state))}>
                                {item.workflow_state}
                              </span>
                            </TableCell>
                            <TableCell className="px-4 py-3 text-xs text-zinc-500 dark:text-zinc-400">
                              {new Date(item.creation).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                            </TableCell>
                            <TableCell className="px-4 py-3 text-xs text-zinc-500 dark:text-zinc-400">
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between px-4 py-3 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-900/10">
                        <div className="text-xs text-zinc-500 dark:text-zinc-400">
                          Showing <span className="font-semibold">{((currentPage - 1) * itemsPerPage) + 1}</span> to <span className="font-semibold">{Math.min(currentPage * itemsPerPage, filteredItems.length)}</span> of <span className="font-semibold">{filteredItems.length}</span> submissions
                        </div>
