...
const FrappeCard = ({
  ...
  <div
    className={cn(
      "bg-white dark:bg-zinc-900 rounded-xl border border-zinc-300 dark:border-zinc-700 shadow-sm",
      className,
    )}
  >
    {children}
...
const FrappeButton = ({
  ...
  <button
    ...
    className={cn(
      ...
      variant === "primary" &&
        "bg-[#D97757] text-white hover:bg-[#D97757] shadow-md hover:shadow-lg border border-[#D97757]",
      variant === "ghost" &&
      ...
      variant === "action" &&
        "bg-[#D97757] text-white font-bold hover:bg-[#D97757] shadow-md hover:shadow-lg border-2 border-[#D97757]",
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-900 dark:text-zinc-100 mt-0.5">
                View all processed and forwarded documents.
              </p>
            </div>
          </div>
        </FrappeCard>
            ...
            <div className="flex flex-1 items-center gap-4 w-full">
              ...
              <div className="flex items-center gap-2">
                <label
                  ...
                  className="font-bold text-zinc-900 dark:text-zinc-100 uppercase text-sm whitespace-nowrap hidden md:block"
                >
                  Filter:
                </label>
                <select
            ...
            <table className="w-full divide-y divide-gray-300">
              <thead className="bg-zinc-200 dark:bg-zinc-700">
                <tr className="divide-x divide-gray-300">
                  <th className="p-3 text-left font-bold text-zinc-900 dark:text-zinc-100 text-sm">
                    Status
                  </th>
                  <th className="p-3 text-left font-bold text-zinc-900 dark:text-zinc-100 text-sm">
                    Module
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                {paginatedTasks.length > 0 ? (
                  paginatedTasks.map((task) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-sm font-mono text-zinc-900 dark:text-zinc-100">
                        {task.id.length > 25
                          ? `${task.id.substring(0, 25)}...`
                          : task.id}
                      </td>
                      <td className="p-4 text-sm font-mono text-zinc-900 dark:text-zinc-100">
                        {task.creation
                          ? new Date(task.creation).toLocaleDateString("en-IN")
                          : "-"}
                      </td>
                      <td className="p-4 text-sm font-mono text-zinc-900 dark:text-zinc-100">
                        {task.modified
                          ? new Date(task.modified).toLocaleDateString("en-IN")
                          : "-"}
                      </td>
                      <td className="p-4 text-sm text-zinc-900 dark:text-zinc-100">
                        {task.owner.length > 20
                        ...
                        <FrappeButton
                          ...
                          onClick={(e) => {
                            ...
                            if (task.doctype === "Fund Received") {
                              ...
                              navigate(
                                `/task-registry/${task.doctype}/${task.id}`,
                              );
                            }
                          }}
                  ...
                  <tr>
                    <td
                      colSpan={8}
                      className="p-8 text-center text-zinc-900 dark:text-zinc-100 font-bold"
                    >
                      {isLoading
                        ? "Loading documents..."
            ...
            <div className="p-4 border-t border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 flex justify-between items-center">
              <div>
                <div className="text-sm text-zinc-900 dark:text-zinc-100 font-medium">
                  ...
                  {indexOfFirstTask + currentCount} of {totalCount} entries
                </div>
              </div>
