...
// //                 <p className="text-gray-700 font-mono mt-1">ID: {projectName} | Status: <span className="font-bold text-black">{data?.sanction_workflow_status || "Draft"}</span></p>
// //               </div>
// //             </div>
...
//     { revalidateOnFocus: true }
//   );
//   // Add this hook in your component (alongside other hooks)
...
//       // frappe.msgprint("Sanction submitted successfully!");
//       refetchSanctions(); // Refresh the sanction list
//     } catch (error: any) {
//       console.error("Error submitting sanction:", error);
//       // frappe.throw(`Failed to submit sanction: ${error.message}`);
//     }
//   },
...
//         // Check if sanction is in draft status
//         const isDraft = sanction.sanction_workflow_status?.toLowerCase() === 'draft';

//         return (
//           <NeoCard key={sanction.name} className="space-y-6">
//             {/* === MODIFIED HEADER WITH STATUS AND SUBMIT BUTTON === */}
//             <div className="pb-4 border-b-2 border-black">
...
//                     <span>
//                       Status: <span className={cn("font-bold uppercase px-2 py-1 rounded-md text-xs", getStatusBadgeClass(sanction.sanction_workflow_status))}>{sanction.sanction_workflow_status || 'DRAFT'}</span>
//                     </span>
//                     <span>
//                       Letter No: <span className="font-semibold text-gray-800">{sanction.sanctioned_letter_no}</span>
//                     </span>
...
//                       className="bg-green-300 hover:bg-green-400 whitespace-nowrap flex items-center gap-2"
//                     >
...
import {
  ...
  ChevronDown, CreditCard, Upload, ShoppingCart, Users, Plane, Settings,
  ZapIcon // Icon for the new Quick Actions tab
} from "lucide-react";
import { cn } from "@/lib/utils";
...
const QuickActions = () => {
  ...
  const ActionButton = ({ children }: { children: React.ReactNode }) => (
    <button className={cn(
      "w-full justify-start text-left text-sm font-semibold text-gray-900",
      "px-4 py-3 border-2 border-gray-900 rounded-lg bg-white",
      "shadow-[2px_2px_0px_rgba(0,0,0,0.1)] transition-all duration-150",
          ...
          {groups.map((group) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center gap-2 px-5 py-3 font-semibold text-sm transition-all duration-150 border-b-2 -mb-2",
                  isActive
                    ? "bg-slate-600 text-white border-slate-600"
                    : "bg-white text-gray-900 border-gray-200 hover:bg-stone-50"
                )}
              >
        ...
        <header className="mb-8 p-4 bg-white border-2 border-black rounded-md shadow-[4px_4px_0px_rgba(0,0,0,0.25)]">
          <div className="flex items-start sm:items-center justify-between flex-col sm:flex-row gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-extrabold text-black">{data?.project_title || "Project Details"}</h1>
                <p className="text-gray-700 font-mono mt-1">ID: {projectName} | Status: <span className="font-bold text-black">{data?.workflow_status || "Draft"}</span></p>
              </div>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
              {isCurrentUserPI && (
