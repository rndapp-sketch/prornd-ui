...
export const isAnnualMaintenanceContractIndent = (indentType: any) => {
    ...
    return normalized.includes("annual maintenance contract") || normalized === "amc";
};
...
export const getPoTableConfig = (indentType: any) => {
    ...
    return {
        ...
        columns: [
            ...
            { key: "item_quantity", label: "Quantity", align: "center" },
            { key: "item_unit_price", label: "Estimated Rate (₹/item)", align: "right" },
            { key: "item_discount_percent", label: "Discount (%)", align: "right" },
            { key: "item_gst_percent", label: "GST (%)", align: "right" },
...
export const getAmcPoTotal = (poData: Record<string, any>) => {
    const rows = Array.isArray(poData.amc_po_table)
        ? poData.amc_po_table
        : [];
...
export const getAmcPoGrandTotal = (poData: Record<string, any>) => {
    const total = getAmcPoTotal(poData);
    return Math.round((total + getAmcPoGstAmount(poData) + Number.EPSILON) * 100) / 100;
};

const getAmcPoTableHtml = (poData: Record<string, any>) => {
    const sourceIndentType =
        poData.po_source_indent_type || poData.icss_indent_type || poData.indent_type;
    const rows = Array.isArray(poData.amc_po_table)
        ? poData.amc_po_table
        : [];

    if (!isAnnualMaintenanceContractIndent(sourceIndentType) || rows.length === 0) {
        return "";
    }
...
export function generatePOHtml(poData: Record<string, any>): string {
    const sourceIndentType =
        poData.po_source_indent_type || poData.icss_indent_type || poData.indent_type;
    const variantCopy = getPoVariantCopy(
        sourceIndentType,
    );
    const tableConfig = getPoTableConfig(
        sourceIndentType,
    );
    ...
    return poTemplate
        ...
        .replace(
            ...
            poData.letterhead_user_email || poData.po_staff_email || poData.staff_email || "",
        )
        .replace("{{PO_HEADER_NOTE}}", poHeaderNote || variantCopy.headerNote)
        .replace("{{INTRO_PARAGRAPH}}", isAmcPo ? amcIntroParagraph : poData.po_intro_paragraph || variantCopy.introParagraph)
        .replace("{{ITEM_TABLE_BLOCK}}", itemTableBlock)
        .replace("{{AMC_PO_TABLE}}", amcPoTableHtml)
        .replace("{{AMOUNT_IN_WORDS}}", poData.amount_in_words || (amountForWords ? fmt(amountForWords) : ""))
        .replace("{{ACCOUNT_HEAD}}", poData.ss_account_head_label || poData.ss_account_head || "")
        .replace("{{PROJECT_NO}}", poData.project_no || "")
        .replace("{{CHECKED_BY_NAME}}", poData.checked_by_name || poData.po_staff_name || "")
        .replace("{{CHECKED_BY_DESIGNATION}}", poData.checked_by_designation || poData.po_staff_designation || "")
