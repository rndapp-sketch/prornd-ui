...
//     dp_total_price?: number | string;
//     item_discount?: number | string;
//     item_gst?: number | string;
// };
...
//             console.log(`  ${key} (${formData[key].length} rows):`, formData[key][0] ?? '(empty)');
//         }
//     }
//     console.groupEnd();

//     // ── Item rows (table_hsrb) ────────────────────────────────────────────────
//     const rows: P11ItemRow[] = Array.isArray(formData.table_hsrb) ? formData.table_hsrb as P11ItemRow[] : [];

//     let pcMembers: PCMember[] = [];
...
//             console.warn('[p11Print] table_teqd not found. Keys available:', Object.keys(formData));
//         }
//     }
...
//     const ownerEmail = String(formData.owner || '');
//     const matchedIndenter = pcMembers.find(m => (m.webmail_id || '').toLowerCase().trim() === ownerEmail.toLowerCase().trim());
...
//     const indenterName = formData.applicant_name || matchedIndenter?.pc_name || pcMembers[0]?.pc_name || emailToName;

//     // ── Item rows HTML ────────────────────────────────────────────────────────
//     const itemRows = rows.map((row, i) => {
//         const lines: string[] = [];
//         if (row.item_name) lines.push(`<strong>Item Name:</strong> ${row.item_name}`);
//         if (row.item_make) lines.push(`<strong>Make:</strong> ${row.item_make}`);
//         if (row.item_model) lines.push(`<strong>Model:</strong> ${row.item_model}`);
//         if (row.item_description) lines.push(`<strong>Description:</strong> ${row.item_description}`);
...
//     const totalDiscount = rows.reduce((s, r) => s + (Number(r.item_discount) || 0), 0);
...
//         (rows.length > 0 ? rows[0].item_name || '' : '');
...
//     const sigSource = pcMembers.length > 0
//         ? pcMembers
//         : [{ pc_name: indenterName, webmail_id: formData.owner || '', designation: '' }];
...
//     const signatureBlocks = `<div style="width:100%;font-size:0;">`
//         + sigSource.map((m) =>
...
export function generateP11Html(formData: Record<string, any>): string {
    ...
    // Fallback: Try to clean the email to make it look like a name if all else fails
    const emailToName = ownerEmail ? ownerEmail.split('@')[0].split('.').map((s: string) => s.charAt(0).toUpperCase() + s.slice(1)).join(' ') : '';
