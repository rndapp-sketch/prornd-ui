...
const MemoizedFormField = memo(({
    ...
    const renderInput = () => {
        switch (field.fieldtype) {
                ...
                return (
                    <label className={cn(
                        ...
                        <div className="relative flex items-center mt-0.5">
                            ...
                            <svg className="absolute inset-0 w-4 h-4 text-white dark:text-zinc-900 opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            case 'Read Only':
                return (
                    <div className="flex h-10 w-full rounded-md border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 px-3 py-2 text-sm text-zinc-700 dark:text-zinc-300">
    ...
    if (field.fieldtype === 'Check') {
        return (
            <div className="space-y-2">
                {renderInput()}
                {field.description && (
                    <p className="text-[0.8rem] text-zinc-500 dark:text-zinc-400 ml-6">{field.description}</p>
                )}
            </div>
        );
    }
    ...
    return (
        <div className="space-y-2">
            <label htmlFor={field.fieldname} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-zinc-700 dark:text-zinc-300 block mb-2">
                {field.label}
                {isMandatory && <span className="text-red-500 dark:text-red-900 ml-1">*</span>}
            </label>
            {renderInput()}
            {field.description && (
                <p className="text-[0.8rem] text-zinc-500 dark:text-zinc-400 mt-1">{field.description}</p>
            )}
        </div>
    );
});
