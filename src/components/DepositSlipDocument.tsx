...
export const DepositSlipDocument: React.FC<DepositSlipDocumentProps> = ({ depositSlip, type = 'research_rnd' }) => {
    ...
    const amountInclusive = parseFloat(depositSlip.amount_inclusive_of_gst || depositSlip.amount_inclusive_gst_capital || depositSlip.total_amount || 0);
    const incomeTaxTds = parseFloat(depositSlip.income_tax_tds || 0);
    const gstTds = parseFloat(depositSlip.gst_tds || 0);

    // 09 Amount Inclusive of GST towards Capital Component - (10 Income Tax TDS + 11 GST TDS) = 12 Amount Actually Received In Bank A/C
    const amountActuallyReceived = amountInclusive - (incomeTaxTds + gstTds);

    const cgst = parseFloat(depositSlip.cgst_amount || depositSlip.cgst || depositSlip.cgst_9 || 0);
    const sgst = parseFloat(depositSlip.sgst_amount || depositSlip.sgst || depositSlip.sgst_9 || 0);
    const igst = parseFloat(depositSlip.igst_18 || depositSlip.igst_amount || depositSlip.igst || 0);

    let calculatedConsultancyFee;

    // Condition 1: If IGST has a value not zero
    if (igst > 0) {
        calculatedConsultancyFee = amountActuallyReceived - igst;
    }
    // Condition 2: If IGST is zero, but CGST or SGST has a value not zero
    else if (cgst > 0 || sgst > 0) {
        calculatedConsultancyFee = amountActuallyReceived - (cgst + sgst);
    }
    // Fallback: No GST values exist
    else {
        calculatedConsultancyFee = depositSlip.consultancy_fee_x || depositSlip.balance_after_gst || amountActuallyReceived;
    }

    const hasConsultancyFee = calculatedConsultancyFee !== undefined && calculatedConsultancyFee !== null && calculatedConsultancyFee !== '';
    ...
    const renderCreditItems = () => {
        ...
        if (Array.isArray(depositSlip.credit_distribution) && depositSlip.credit_distribution.length > 0) {
            depositSlip.credit_distribution.forEach((item: any) => {
                ...
                // Strip out trailing date strings from the label itself (e.g., "IDF / 2026-07-02" -> "IDF")
                label = label.replace(/\s*\/\s*\d{4}-\d{2}-\d{2}.*/, '').trim();
                const recipient = (item.recipient_name || '').trim();
                const isDateString = /^\d{4}-\d{2}-\d{2}/.test(recipient); // Just check if it starts with a date
            ...
            <table className="w-full border-collapse text-sm mb-3" style={{ tableLayout: 'fixed' }}>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td colSpan={2} className="border border-black p-1 text-right">
                            {formatCurrency(amountActuallyReceived)}
                        </td>
                    </tr>

                    {/* CGST */}
                    {(depositSlip.cgst_amount !== undefined || depositSlip.cgst !== undefined || depositSlip.cgst_9 !== undefined) ? (
                        <tr>
                            <td className="border border-black p-1 text-center">{getRowNum()}</td>
                    ...
                    {/* SGST */}
                    {(depositSlip.sgst_amount !== undefined || depositSlip.sgst !== undefined || depositSlip.sgst_9 !== undefined) ? (
                        <tr>
                            <td className="border border-black p-1 text-center">{getRowNum()}</td>
                    ...
                    {/* IGST */}
                    {(depositSlip.igst_18 !== undefined || depositSlip.igst_amount !== undefined || depositSlip.igst !== undefined) ? (
                        <tr>
                            <td className="border border-black p-1 text-center">{getRowNum()}</td>
                    ...
                    {hasConsultancyFee ? (
                        <tr>
                            ...
                            <td colSpan={2} className="border border-black p-1 text-right">
                                {formatCurrency(calculatedConsultancyFee)}
                            </td>
                        </tr>
                    ) : null}
                    ...
                    <tr>
                        ...
                        <td className="border border-black p-1">
                            {type === 'consultancy_d' ? 'Total Overhead (0.1 * Y + 0.1 * Z)' :
                                (type === 'consultancy_e' || type === 'consultancy_t')
                                    ? `Overhead (${type === 'consultancy_t' ? 0.7 : 0.3} × X)`
                                    : 'Overhead Amount @ 15% (inclusive)'}
                        </td>
                        <td colSpan={2} className="border border-black p-1 text-right">{formatCurrency(depositSlip.overhead_amount)}</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td colSpan={2} className="border border-black p-1 text-right font-bold">
                            {formatCurrency(
                                (calculatedConsultancyFee || 0) - (depositSlip.overhead_amount || 0)
                            )}
                        </td>
                    </tr>
