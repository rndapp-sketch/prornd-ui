...
export function AppSidebar() {
  ...
  const hasOverviewAccess = roles?.some((r) =>
    [
      "Director",
      "Dean, RnD",
      "Ado_RnD",
      ...
      ? [
          {
            ...
            subMenu: [
              ...
              { label: "PI Projects", path: "/director-dashboard?view=PI" },
            ],
          },
        ]
      : []),
  ...
  const handleMenuItemClick = (item: MenuItem) => {
    if (item.subMenu) {
      ...
      // Navigate if there's a path for the main menu item with a submenu
      if (item.path && item.label === "Overview") {
        navigate(item.path);
      }
    } else if (item.label === "Home" || item.label === "Overview") {
      navigate("/dashboard");
    } else if (item.path) {
      navigate(item.path);
    }
      ...
      >
        ...
        >
          <div
            className={cn(
              "flex items-center",
              state === "expanded" ? "gap-2 w-full" : "justify-center",
            )}
          >
            ...
            <SidebarMenu className="space-y-1">
              {menuItems.map((item) => {
                const isAnySubMenuActive =
                  item.subMenu?.some((sub) => isActivePath(sub.path)) ?? false;
                const isActive =
                  (item.path && isActivePath(item.path)) || isAnySubMenuActive;
