    ...
    useEffect(() => {
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                const headRes = await fetch('/api/v2/document/Budget Head?fields=["budget_head","id"]&order_by=id asc');
                ...
                results.forEach(({ head, headId, data }) => {
                    ...
                    const headReceived = data.reduce((sum: number, item: any) => sum + (item.fundReceivedAmount || 0), 0);
                    const headCommitted = data.reduce((sum: number, item: any) => sum + (item.commitAmount || 0), 0);
